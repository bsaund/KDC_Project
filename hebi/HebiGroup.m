classdef (Sealed) HebiGroup < handle
    ...
    %   getFeedbackFrequency - returns the current feedback request rate
    %   setFeedbackFrequency - sets the feedback request rate
    %   getCommandLifetime   - returns the current command lifetime
    ...
    %   set                  - sends synchronized commands and settings
    ...
    methods(Access = public)
        ...
        function out = getNumModules(this, varargin)
            ...
            %
            %   Example
            %      % Command all-zero torques
            %      cmd = CommandStruct()
            %      cmd.torque = zeros(1, group.getNumModules())
            %      group.set(cmd);
        ...
        function this = setCommandLifetime(this, varargin)
            ...
            %   unexpected behavior when sending torques and velocities.
            %
            %   Example
        ...
        function [] = set(this, varargin)
            %set sends commands and settings to modules.
            %
            ...
            %   such as positions, velocities, or torques. It expects a CommandStruct.
            ...
            %      group.set('cmd', cmd); % enable
            %      cmd.velocity = [];
            %      group.set(cmd); % disable (omit flag)
            %
            ...
            %
            %   Example
            %      % set torque limits of +/- 1.5 Nm
            %      n = group.getNumModules();
            %      limit = 1.5;
            %      gains = GainStruct();
            %      gains.torqueMaxOutput = ones(1,n) * limit;
            %      gains.torqueMinOutput = ones(1,n) * -limit;
            ...
            %
            %   Example
            %      group.set('led', 'red'); % set all to red
            %      group.set('led', [0 0 1]); % set all to blue [r g b] [0-1]
            %      group.set('led', []); % reset to default mode
            %
            ...
            %      group.set('family', 'MyRobot', 'led', 'r');
            %
            %   See also HebiGroup, CommandStruct, GainStruct
            set(this.obj, varargin{:});
        end
        ...
        function out = getNextFeedback(this, varargin)
            ...
            %   immediately. If no feedback is available, this function waits and
            %   returns once the next feedback arrives.
            %
            %   If no feedback arrives, the function eventually times out and returns
            %   empty ([]). If this happens, make sure that the hardware is turned on
            %   and that the feedback request rate is set.
            %
            %   'View' Parameter ('Feedback' only)
            %      'Simple'   returns basic feedback. This is appropriate for most
            %                 users. (default)
        ...
        function out = getInfo(this, varargin)
            %getInfo returns meta information such as names
            %
            %   This method returns the latest received info. It returns
            %   immediately and may return the same result more than once.
            %   Note that info structs get populated by lookup requests,
            %   which usually get requested at a lower rate than feedback.
            %   If info is not available, this method will return empty ([]).
            %
            %   The info struct contains meta information about modules, such
            %   as their names, versions, network settings, and serial numbers.
            %
            %   Due to language constraints, there is an empty element appended
            %   to each field, if the group consists of more than one module.
            %   This extra element can be removed using a selection mask.
            %
            %   Example
            %      % Remove empty element from mac addresses
            %      info = group.getInfo();
            %      selection = 1:info.numModules;
            %      macAddress = info.macAddress(selection)
            %
            %   If info returns empty or do not update as expected, confirm
            %   that the info polling rate is larger than zero.
            %
            %   Example
            %      % Display the lookup polling rate
            %      HebiLookup
            %
            %   Example
            %      % Find names of all modules
            %      info = group.getInfo()
            %      names = info.name(1:info.numModules);
            %      display(names);
            %
            %   See also HebiGroup, HebiLookup.setLookupFrequency
        ...
        function out = startLog(this, varargin)
            ...
            %       plot(log.time, log.position);
            %
            %   See also HebiGroup, stopLog, stopLogFull, HebiUtils.convertGroupLog
        ...
        function out = stopLog(this, varargin)
            ...
            %
            %   See also HebiGroup, startLog, stopLogFull
