...
% allow a different transformation between each phase/swing state to test,
% with the assumption that there will be a short full stance period where
...
phasesToTest(end,:) = phase0 + 2*pi;%-.01;
...
B( (length(oddInds)-1 + length(evenInds)-1+1) :5: end) = pi/2;
