...
% one stance transformation per step phase, just before the leg takes off
% assumption that there will be a short full stance period where
% the weight its shifted.
...
phasesToTest = zeros(nStanceLegs, nStanceLegs);
swingAtPhasesToTest = eye(nStanceLegs);
[~,inds ] = sort(stepOrder);
swingAtPhasesToTest = swingAtPhasesToTest(:,inds);
% % swingAtPhasesToTest = circshift(swingAtPhasesToTest, -1);
for k = 1:nStanceLegs
   phasesToTest(k,:)  = phase0 + k*swingPhaseLength;
end
...
transformLBMat(1,:) = -pi/8;
% second row: thetaY
transformUBMat(2,:) = pi/10;
transformLBMat(2,:) = -pi/10;
% third row: rB_P x
transformUBMat(3,:) = params.W/4; % was /4
transformLBMat(3,:) = -params.W/4; % was /4
% fourth row: rB_P y
transformUBMat(4,:) = params.L/2; % was /4
transformLBMat(4,:) = -params.L/2; % was /4
% fifth row: rB_P z
transformUBMat(5,:) = .25;
transformLBMat(5,:) = .1;
...
A = zeros(length(oddInds)-1 + length(evenInds)-1 + nPhases*4, length(UB));
...
% too big.
a = zeros(nPhases, length(transforms)); 
a(1,1) = 1; a(1,6) = -1;
for k= 2:nPhases
...
state0(state0>UB.') = UB(state0>UB.');
state0(state0<LB.') = UB(state0<LB.');
stateOpt = cmaes('costWalk10cmaes', state0.', sigma, opts); 
 save('lastResultsCMAES.mat');
