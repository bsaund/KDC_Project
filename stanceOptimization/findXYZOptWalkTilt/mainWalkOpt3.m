...
stepLength = .07; % might later be an optimized parameter
...
UB = [reshape(UBMat, [1 2*nStanceLegs]) .15 .4 .2];
LB = [reshape(LBMat, [1 2*nStanceLegs]) -.15 -.4 .05];
...
Beq = [];
options = optimset('TolCon', 1e-3, 'TolFun', 1e-3, 'MaxFunEvals', 4000 );% for speed
costFun = @costWalk3;
