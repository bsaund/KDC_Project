...
xy0 = reshape(xyz0(1:2,:), [1, 2*nLegs]); % initial value: all legs 
% form of: [x1 y1 x2 y2 ...]
...
planey = .2;
planec = .2;
state0 = [xy0 planex planey planec ];
...
stanceLegBaseXY = legBaseXY;
oddInds =find(mod(allLegs,2)==1); % the indexes of the odd legs in stanceLegs
evenInds= find(mod(allLegs,2)==0);
UBMat = Inf(2,nLegs); LBMat = -UBMat;
...
UBMat(2,:) = stanceLegBaseXY(2,:) +.3; % not too far forwards
LBMat(2,:) = stanceLegBaseXY(2,:) -.3; % not too far back
UBMat(2,extraLegs) = .45; % extra legs not too far forwards
LBMat(2,extraLegs) = .2; % extra legs not under body

% give a smaller limit on the x direction tilt than the y direction.
planeUB = [ .02 .5 .2];
planeLB = [-.02 -.5 .05];
UB = [reshape(UBMat, [1 2*nLegs]) planeUB];
LB = [reshape(LBMat, [1 2*nLegs]) planeLB];
