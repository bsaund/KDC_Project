...
stanceLegs = [2 3 6]; % array of legs that are in the air, stretched out far
zd = .2; % the minimum body height
...
options = optimset('TolCon', 1e-3, 'TolFun', 1e-3);% for speed
 ...
 thOpt = fmincon(@cost1,th0,A,B,Aeq,Beq,LB,UB,@nonlincon1,options);
...
bodyCoM = ...
 ...
 % plotting
  ...
  set(gcf, 'position', [10 200 1000 700]);
