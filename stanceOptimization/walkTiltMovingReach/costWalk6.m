function f = costWalk6(state)

global kin stanceLegs extraLegs  plt A evals nLegs
...
extraLegPhaseOrder = circshift(ceil((1:nPhases)/2),[1 -1]);
...
%     xyz = xyz0;
    ...
    % change the xyz of the extra legs
       xyz(1:2,extraLegs) = ...
           reshape( xyExtraMat(extraLegPhaseOrder(k),:),...
            [2 nExtraLegs] );% this will be which row of the xyExtra matrix to use
...
% add on the feet as points of interest
feet = kin.getLegPositions(thIK);
x = [x feet(:,2:2:6)];
IP = x' * x;
...
footOverlap= A*[xyStepT zeros(1,nPhases*nExtraLegs) 0 0 0].';
costPhases(k) =  costPhases(k) ...
