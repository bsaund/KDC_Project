...
xyzStance0 = xyz; % save original configuration for modification
 ...
 if makeVideo
     v = VideoWriter(['gaitVid5legReach1.avi']);
      open(v);
  end
...
% get the foot placement points
    ...
    % get extra legs
%      Vq = interp1(X,V,Xq)
     xyExtraList = interp1((0:2*pi/nStanceLegs:2*pi),xyExtraMat([1:end, 1],:),mod(t,2*pi));
     xyz(1:2, extraLegs) = reshape(xyExtraList, [2,nExtraLegs]);
