...
close all;
 ...
 %% plot the results
xyStepOpt = reshape(stateOpt(1:nStanceLegs*2), [2 nStanceLegs]);
xyExtraMat = reshape(stateOpt(nStanceLegs*2+1 : nStanceLegs*2 + nPhases*nExtraLegs ), [nPhases/2, nExtraLegs*2]);
planexyc = stateOpt(end-2:end);

xyz = zeros(3,6);
xyz(1:2,stanceLegs) = xyStepOpt; 
